...
class Team_project_model extends CI_Model {

    var $tn = "team-projects";
...
//        $this->load->model("note_tag_model");
     ...
     * @return type
     */
    public function getTeamProjectLinkById($id) {
        $query = $this->db->get_where($this->tn, array('id' => $id));
        return $query->row();
    }
    
    public function getTeamProjectLinks() {
     ...
     * @return null
     */
    public function getTeamProjectLinksbyTeamId($teamId, $limit = null, $offset = 0, $count = false) {
...
//        echo $this->db->last_query();
     ...
     * @return null
     */
    public function getTeamProjectLinksbyProjectId($teamId, $limit = null, $offset = 0, $count = false) {
//        echo "userId: ".$userId." >> limit: ".$limit . " >> offset: ". $offset ." >> count: ". $count;
        $this->db->order_by("created_date", "desc");
        if (null == $limit) {
            $query = $this->db->get_where($this->tn);
...
//        echo $this->db->last_query();
    ...
    public function getTeamProjectLinksByIds($userId, $teamProjectIds) {
        if ($userId == null) {
            return null;
        }
        if ($noteIds == null) {
